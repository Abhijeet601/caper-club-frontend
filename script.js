...
const FACE_SCAN_DEBOUNCE_MS = 400;
...
async function doorLockNow() {
  if (!ensureAdmin()) return;
  if (doorControlInFlight) return;

  try {
    doorControlInFlight = true;
    if (doorUnlockAutoTimerId) {
      clearTimeout(doorUnlockAutoTimerId);
      doorUnlockAutoTimerId = null;
    }

    await api('/door/lock', { method: 'POST', body: {} });
    await syncDoorState({ silent: true });
    toast('Door locked.', 'success');
  } catch (err) {
    handleErr(err, { toast: true });
  } finally {
    doorControlInFlight = false;
  }
}

async function doorUnlockFor5s() {
  if (!ensureAdmin()) return;
  if (doorControlInFlight) return;

  // These endpoints are protected by X-Door-Key in backend/door_lock_routes.py.
  // The dashboard backend uses JWT; the door-lock router uses a separate door API key.
  const key = String(localStorage.getItem('doorApiKey') || '').trim() || 'caperclub-door-2026';
  ...
  try {
    ...
    const unlockHeaders = { 'X-Door-Key': key };
    await api('/door/unlock', { method: 'POST', headers: unlockHeaders, body: {} });
    await syncDoorState({ silent: true });
    toast('Door unlocked (auto-lock in 5s).', 'success');

    doorUnlockAutoTimerId = setTimeout(async () => {
      doorUnlockAutoTimerId = null;
      try {
        const lockHeaders = { 'X-Door-Key': key };
        await api('/door/lock', { method: 'POST', headers: lockHeaders, body: {} });
        await syncDoorState({ silent: true });
      } catch (err) {
